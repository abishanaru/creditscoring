...
```{r}
libraries = c("readr", "tidyverse", "ggplot2", "dplyr", "reshape2", "plotly", "caret", "DescTools", "pROC", "ROCR", "ROSE")
...
```{r}
loan_sample$Status <- as.factor(loan_sample$Status)
```
In the code the "Status" variable is converted to a factor to properly represent the binary outcome of credit approval (1 for 'Yes', 0 for 'No') in the upcoming predictive model. This ensures the model treats "Status" as a categorical, not numerical, variable.
