...
```{r}
loan_data_balanced <- ovun.sample(Status ~ ., data=loan_sample, method = "under")
...
```{r}
# Create a copy of balanced data
loan_data_balanced_copy <-  loan_sample_balanced
```
...
# Assuming your dataset is named 'new_balance_data' and the target variable column is 'Status'
# Split the data into training (70%) and testing (30%) sets
splitIndex <- createDataPartition(new_balance_data$Status, p = 0.7, list = FALSE)
training_set <- new_balance_data[splitIndex,]
testing_set <- new_balance_data[-splitIndex,]
```
