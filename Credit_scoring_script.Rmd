...
```{r}
# First, split the data into two subsets based on the target variable levels.
defaulters <- loan_sample[loan_sample$Status == 1, ]
non_defaulters <- loan_sample[loan_sample$Status == 0, ]

# Now, undersample the larger group to the size of the smaller one.
set.seed(123) # Set seed for reproducibility
non_defaulters_balanced <- non_defaulters[sample(1:nrow(non_defaulters), size = nrow(defaulters)), ]

# Combine the undersampled non-defaulters with the defaulters to create a balanced dataset.
loan_sample_balanced <- rbind(defaulters, non_defaulters_balanced)


# Check the balance of the new dataset
table(loan_sample_balanced$Status)

# Plot the balance
library(ggplot2)
ggplot(loan_sample_balanced, aes(x = as.factor(Status))) +
  geom_bar() +
  labs(x = "Status", y = "Count", title = "Count of loan status in the balanced dataset") +
  theme_minimal()
```
...
```{r}
set.seed(7)
data_balanced <- ovun.sample(Status ~ ., data=loan_data, method = "under")
